...
public class StoreInventoryInputBridge : MonoBehaviour
{
    ...
    void BindShopUiCloseHook()
    {
        UnbindShopUiCloseHook();

        boundShopUI = FindObjectOfType<ShopUI>(true);
        if (boundShopUI == null)
            return;

        boundShopUI.Closed += HandleShopUiClosed;
    }

    void UnbindShopUiCloseHook()
    {
        if (boundShopUI == null)
            return;
