...
namespace StoreAndInventory
{
    ...
    public class ShopService : MonoBehaviour
    {
        ...
        public BuyResult TryBuy(string itemId, int count = 1)
        {
            ...
            if (wallet == null || !wallet.Has(currency, totalPrice))
            {
                return BuyResult.NoCurrency;
            }

            if (!wallet.TrySpend(currency, totalPrice, $"buy {itemId} x{count}"))
                return BuyResult.NoCurrency;

            if (inventory == null)
            {
                wallet.Add(currency, totalPrice, $"rollback buy {itemId} x{count}");
                Debug.LogWarning($"[ShopService] TryBuy → NoSpace (rollback {currency}={totalPrice}): inventory is null");
            ...
            {
                wallet.Add(currency, totalPrice, $"rollback buy {itemId} x{count}");
                Debug.LogWarning($"[ShopService] TryBuy → NoSpace (rollback {currency}={totalPrice}): {addMsg}");
                return BuyResult.NoSpace;
            }
        ...
        public SellResult TrySell(int bagSlotIndex, int count, out string message)
        {
            ...
            if (wallet != null)
                wallet.Add(def.currencyId, totalPrice, $"sell {def.id} x{count}");
        ...
        int GetRemainingStockForEntry(ShopEntry entry)
        {
            ...
            if (stockById.TryGetValue(entry.item.id, out var stockEntry))
