...
public class PlayerBattleEntity : BattleEntity
{
    ...
    private void OnEnable()
    {
        ...
        // 从存档加载装备的大招
        string savedUltName = SaveManager.Instance.LoadEquippedUltimate();
        if (!string.IsNullOrEmpty(savedUltName))
        {
            foreach (var ult in unlockedUltimates)
            {
                if (ult.ultimateName == savedUltName)
                {
        ...
        {
            equippedUltimate = unlockedUltimates[0];
            Debug.Log($"<color=orange>[大招系统] 已自动装备第一奥义: {equippedUltimate.ultimateName}</color>");
        }

        if (InputManager.Instance?.Battle != null)
        {
            InputManager.Instance.Battle.OnParryPressed += OnParryPressedReceived;
    ...
    public void EquipUltimate(UltimateSkill skill)
    {
        if (unlockedUltimates.Contains(skill))
        {
            ...
            SaveManager.Instance.SaveEquippedUltimate(skill.ultimateName);
    ...
    public void SwitchForm(int targetFormIndex)
    {
        ...
        // 扣除并判定共享 AP
        if (BattleResourceManager.Instance.sharedAP >= targetForm.apCostToSwitch)
        {
            BattleResourceManager.Instance.sharedAP -= targetForm.apCostToSwitch;
    ...
    {
        if (BattleResourceManager.Instance.sharedMP >= skill.mpCost)
        {
            BattleResourceManager.Instance.sharedMP -= skill.mpCost;
    ...
    public void CastUltimate()
    {
        ...
        if (BattleResourceManager.Instance.sharedUltimateEnergy >= BattleResourceManager.Instance.maxSharedUltimateEnergy)
        {
            // 1. 扣除共享大招能量
            BattleResourceManager.Instance.sharedUltimateEnergy = 0;
            ...
            // 3. 开启特写子弹时间
            BattleEffectManager.Instance.WitchTime(0.4f);
    ...
    public void ExecutePendingSkillDamage()
    {
        ...
        // 技能伤害 = 基础伤害 + 攻击力加成（让 WeakenBuff 等攻击力修正实际影响伤害）
        int effectiveDamage = PendingSkill.baseDamage + Mathf.Max(Stats.attack, 0);
